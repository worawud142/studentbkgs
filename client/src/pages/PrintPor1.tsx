...
export default function PrintPor1() {
  ...
  const { data: attendanceSummaries = [] } = trpc.attendance.getByAssignment.useQuery({ assignmentId: aId });
  const { data: academicYears = [] } = trpc.academicYear.list.useQuery({});
  const academicYear = academicYears.find(y => y.id === academicYearId);

  const saveDoc = trpc.document.save.useMutation({
    onSuccess: (data) => {
      toast.success("บันทึกเอกสารสำเร็จ สามารถดาวน์โหลดได้จากหน้าเอกสาร");
    },
  ...
  const attendanceCount: Record<number, { present: number; absent: number; late: number; total: number }> = {};
  attendanceSummaries.forEach((a) => {
    if (!attendanceCount[a.studentId]) attendanceCount[a.studentId] = { present: 0, absent: 0, late: 0, total: 0 };
    attendanceCount[a.studentId].total++;
    if (a.status === "present") attendanceCount[a.studentId].present++;
    else if (a.status === "absent") attendanceCount[a.studentId].absent++;
    else if (a.status === "late") attendanceCount[a.studentId].late++;
  });
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="no-print bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          <span className="text-slate-700 font-medium">ปพ.1 - {subject?.name}</span>
      ...
      <div className="p-8 flex justify-center">
        <div ref={printRef} className="bg-white shadow-lg print-page" style={{ width: "210mm", minHeight: "297mm", padding: "15mm 20mm", fontFamily: "'IBM Plex Sans Thai', sans-serif", fontSize: "10pt" }}>
          ...
          <div className="text-center mb-6 border-b-2 border-slate-900 pb-4">
            <h1 className="text-xl font-bold">แบบรายงานผลการเรียน</h1>
            <h2 className="text-lg font-bold">ปพ.1 : ระเบียนแสดงผลการเรียน</h2>
            <p className="text-sm mt-1">ระดับมัธยมศึกษา</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div className="space-y-1">
              ...
              <p><span className="font-semibold">หน่วยกิต:</span> {subject?.credits || "-"}</p>
            </div>
            <div className="space-y-1">
              <p><span className="font-semibold">ห้องเรียน:</span> {classroom?.name}</p>
              <p><span className="font-semibold">ปีการศึกษา:</span> {academicYear?.year}{academicYear?.semester ? ` ภาคเรียนที่ ${academicYear.semester}` : ""}</p>
          ...
          <table className="w-full text-xs border-collapse mb-6">
            <thead>
              <tr className="bg-slate-100">
                ...
                <th className="border border-slate-400 px-2 py-1.5 text-center w-12">เกรด</th>
                <th className="border border-slate-400 px-2 py-1.5 text-center w-16">ผล</th>
                <th className="border border-slate-400 px-2 py-1.5 text-center w-16">มา/ขาด</th>
              </tr>
            </thead>
            <tbody>
              {students.map((s, idx) => {
                const g = gradeMap[s.id];
                const att = attendanceCount[s.id];
                return (
                  <tr key={s.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="border border-slate-300 px-2 py-1 text-center font-bold text-blue-700">
                      {g?.grade || "-"}
                    </td>
                    <td className="border border-slate-300 px-2 py-1 text-center text-xs">
                      <span className={g?.result === "pass" ? "text-green-700" : g?.result === "fail" ? "text-red-700" : ""}>
                        {g?.result === "pass" ? "ผ่าน" : g?.result === "fail" ? "ไม่ผ่าน" : "-"}
                      </span>
                    </td>
                    <td className="border border-slate-300 px-2 py-1 text-center text-xs">
                      {att ? `${att.present}/${att.absent}` : "-/-"}
                    </td>
                  </tr>
                );
              })}
          ...
          <div className="mb-6 text-xs">
            <p className="font-semibold mb-1">เกณฑ์การให้เกรด:</p>
            <div className="grid grid-cols-4 gap-2">
              {[["4", "80-100%"], ["3.5", "75-79%"], ["3", "70-74%"], ["2.5", "65-69%"], ["2", "60-64%"], ["1.5", "55-59%"], ["1", "50-54%"], ["0", "0-49%"]].map(([g, r]) => (
                <span key={g} className="text-slate-600">{g} = {r}</span>
              ))}
            </div>
          </div>
